...
async def get_analytics_summary(db: AsyncSession = Depends(get_db)):
    ...
    top_domains = [
        {"domain": domain or "other", "count": count} for domain, count in domain_rows
    ]
...
@cached_route(
    "analytics:trends",
    ttl=300,
    key_params=["metric", "interval", "start_date", "end_date"],
)
async def get_analytics_trends(
    ...
    start_date: str | None = Query(
        None, description="Start date (ISO 8601) or relative like -30d"
    ),
    ...
    if start_date:
        if start_date.startswith("-"):
            ...
            if unit != "d":
                raise HTTPException(
                    status_code=400,
                    detail="Invalid start_date relative format. Use -<days>d (e.g. -30d).",
                )
            try:
            ...
            if days <= 0:
                raise HTTPException(
                    status_code=400,
                    detail="Invalid start_date relative format. Use a positive day value (e.g. -30d).",
                )
    ...
    if metric == "ideas" and len(rows) > 0 and len(rows[0]) == 3:
        ...
        data_points = [
            {"date": row[0].isoformat() if row[0] else None, "value": row[1]}
            for row in rows
        ]
    ...
    domains = [
        {
            ...
            "percentage": round((idea_count / total_ideas) * 100, 1)
            if total_ideas > 0
            else 0.0,
        }
