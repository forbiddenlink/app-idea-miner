...
class ApiClient {
  ...
  async getSimilarClusters(
    ...
    source_cluster: Partial<Cluster>;
  ...
  async removeBookmark(
    ...
    const response = await this.client.delete(
      `/api/v1/bookmarks/${itemType}/${itemId}`,
    );
    return response.data;
  }
  ...
  async getSavedSearches(
    ...
    const response = await this.client.get("/api/v1/saved-searches", {
      params,
    });
    return response.data;
  }
  ...
  async createSavedSearch(
    ...
    const response = await this.client.post("/api/v1/saved-searches", payload);
    return response.data;
  }
