...
import { EmptyAnalytics, ErrorState } from "@/components/EmptyStates";
...
const AnalyticsVisuals = lazy(
  () => import("@/components/analytics/AnalyticsVisuals"),
);
...
function OverviewStats({
  data,
  isLoading,
}: Readonly<{ data: SummaryData | undefined; isLoading: boolean }>) {
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={`skeleton-stat-${i}`}
            className="card h-24 animate-pulse bg-muted/45"
          />
        ))}
      </div>
    );
...
function DomainTable({
  data,
  isLoading,
}: Readonly<{ data: DomainStats[] | undefined; isLoading: boolean }>) {
...
export default function Analytics() {
  ...
  const { data: trends, isLoading: trendsLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.getAnalyticsTrends({
        metric: trendMetric,
        interval: "day",
      });
  ...
  } else if (!summaryLoading && !hasAnalyticsData) {
    analyticsBody = <EmptyAnalytics />;
  } else {
    analyticsBody = (
      <>
