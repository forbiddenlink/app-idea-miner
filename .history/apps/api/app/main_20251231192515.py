...
async def health_check():
    ...
    try:
        ...
        services["database"] = {"status": "up", "latency_ms": db_latency_ms, "message": "Connected"}
    except Exception as e:
    ...
    if redis_client:
        ...
        services["redis"] = {"status": "unknown", "message": "Redis client not configured"}
...
async def metrics():
    ...
    try:
        async with AsyncSessionLocal() as session:
            # Count metrics
            posts_result = await session.execute(select(func.count()).select_from(RawPost))
            ideas_result = await session.execute(
                select(func.count())
                .select_from(IdeaCandidate)
                .where(IdeaCandidate.is_valid == True)
            )
            clusters_result = await session.execute(select(func.count()).select_from(Cluster))
