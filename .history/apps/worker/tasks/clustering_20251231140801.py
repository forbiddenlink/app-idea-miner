...
from packages.core.clustering import cluster_ideas, get_cluster_engine
...
def run_clustering(
    self, min_quality: float = 0.3, min_cluster_size: int = 3, recreate_clusters: bool = True
) -> dict[str, Any]:
    """
    ...
    # Run async logic
    result = asyncio.run(_run_clustering_async(min_quality, min_cluster_size, recreate_clusters))
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            stmt = (
                ...
                .where(IdeaCandidate.is_valid == True, IdeaCandidate.quality_score >= min_quality)
            ...
            cluster_result = cluster_ideas(texts, min_cluster_size=min_cluster_size)
            ...
            for cluster_id in range(cluster_result.n_clusters):
                ...
                avg_sentiment = sum(idea.sentiment_score for idea in cluster_ideas) / idea_count
                avg_quality = sum(idea.quality_score for idea in cluster_ideas) / idea_count
                ...
                clusters_created += 1
                logger.debug(f"Created cluster {cluster_id}: {label} ({idea_count} ideas)")
            ...
            return {
                ...
                "noise_ideas": cluster_result.n_noise,
            }
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            for cluster in clusters:
                ...
                new_trend_score = engine.calculate_trend_score(timestamps, window_days=7)
